...
tempdir = '/autofs/space/bodhi_001/users/mdatko/migraine/p1/tmp/';  % set so that matlab doesn't run into space issues unzipping files in /tmp/

root = '/autofs/space/bodhi_001/users/mdatko/migraine/p1/data/';
bolddir = strcat(root,'nifti/');
...
for j=1:numel(subj_list)
    subj = subj_list{j};
    list_raw = dir(strcat(bolddir,'*',subj,'*pre/func/*bold.nii.gz'));
    ...
    for i = 1:numel(list_raw)
        ...
        outfunc2 = strcat(outpath,'_retro-corrected.nii.nii'); %output file

        if exist(outfunc2,'file') == 2
